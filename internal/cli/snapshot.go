...
// renderSnapshotTable writes the `snapshot list` table, then any reason a
// conversation was not captured.
//
// The notes go BELOW the table rather than in it. A tabwriter column is as
// wide as its widest cell, so a one-line explanation in the CONVERSATION
// column stretches that column to ~140 characters and shoves SEQ, LABEL and
// the rest off the right of any terminal. The single-snapshot output already
// prints its note on a line of its own; this does the same.
func renderSnapshotTable(out io.Writer, snaps []store.Snapshot) error {
	w := newTabWriter(out)
	...
	for _, note := range conversationNotes(snaps) {
		fmt.Fprintf(out, "\nnote: %s\n", note)
	}
	return nil
}
...
//
// A word, never a sentence. The note explaining WHY is one line of prose;
// putting it in a tabwriter column sets that column's width to the length of
// the note and pushes every other column off the screen, at any terminal
// width. It is printed once under the table instead — which is also where the
// single-snapshot output already puts it.
...
// conversationNotes collects the distinct reasons conversations were not
// captured, in the order they first appear. Distinct, because every snapshot
// of a host-placed container carries the same sentence and printing it once
// per row would be the same mess in a different shape.
func conversationNotes(snaps []store.Snapshot) []string {
	var notes []string
	seen := map[string]bool{}
	for _, s := range snaps {
		if s.IncludesConversation || s.Note == "" || seen[s.Note] {
			continue
		}
		seen[s.Note] = true
